...
- Reduce Kafka topics from ~22 to ~8, retaining only execution-boundary events
...
**Choice:** The Gateway handles CRUD synchronously (forward → service → DB → respond). Only execution/lifecycle side-effects travel over Kafka: task commands (`task.run-requested`, `task.review-requested`, `task.stop-requested`) and facts (`step.*`, `run.completed`, `verdict`, `pr.opened`). All other previously-defined topics become in-process events or synchronous calls.
...
Phase 4: Clean Up
1. Remove 6 Dockerfiles, 6 DB init scripts, ~14 Kafka topics
