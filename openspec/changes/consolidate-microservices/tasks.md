...
- [x] 1.1 Create `services/identity/` directory with `cmd/main.go` composition root
- [x] 1.2 Move domain packages from Auth, Orgs, Admin as internal subpackages:
    - `internal/domain/identity/` combining auth, org, admin domains
    - Preserve existing domain/application/infrastructure layering
- [x] 1.3 Consolidate databases: single migration pipeline creating all tables in `identity_db`
...
- [x] 1.4 Replace inter-service Kafka events with in-process event bus
    - `signup.*`, `invite.created`, `workspace.created`, `audit.recorded` become local events
- [x] 1.5 Update Gateway: `UPSTREAM_AUTH` + `UPSTREAM_ORGS` + `UPSTREAM_ADMIN` → `UPSTREAM_IDENTITY`
- [x] 1.6 Run `go vet ./... && go build ./...` — ensure no layer violations, compilation passes
- [x] 1.7 Run existing service tests with hand-rolled fakes to confirm no regressions

## Phase 2: Merge Project + Task + Catalog + Resources → Workspace Service

- [x] 2.1 Create `services/workspace/` directory with `cmd/main.go` composition root
- [x] 2.2 Move domain packages from Project, Task, Catalog, Resources as internal subpackages:
    - `internal/domain/workspace/` combining project, task, catalog, resource domains
- [x] 2.3 Consolidate databases: single migration pipeline creating all tables in `workspace_db`
...
- [x] 2.4 Catalog→Resources projections become local function calls (no Kafka needed)
- [x] 2.5 Task saga coordinator gains direct access to project/skill data (no cross-service calls)
- [x] 2.6 Update Gateway routing for workspace endpoints
- [x] 2.7 Run `go vet ./... && go build ./...` — ensure compilation and layer integrity
- [x] 2.8 Run existing service tests to confirm no regressions

## Phase 3: Merge Agent + Settings → Agent Service

- [x] 3.1 Create `services/agent/` directory with enhanced `cmd/main.go` composition root
- [x] 3.2 Move domain packages from Agent, Settings as internal subpackages:
...
- [x] 4.1 Rename `services/runner/` → `services/executor/` (preserving all internal structure)
- [x] 4.2 Update `cmd/main.go` composition root if needed for new package path
- [x] 4.3 Kafka topics: produces `step`, `run.completed`, `finding`, `verdict`, `pr.opened`
...
- [x] 4.4 Run `go vet ./... && go build ./...` — verify unchanged service compiles
- [x] 4.5 Run existing runner/executor tests

## Phase 5: Gateway BFF simplification

- [x] 5.1 Reduce upstream env vars from 10 to 4:
...
- [x] 5.2 Simplify session composition: single call to Identity Service instead of Auth→Orgs fan-out
- [x] 5.3 Update Gateway health probes to check 4 downstream services instead of 11
- [x] 5.4 Run `go vet ./... && go build ./...` — verify Gateway compiles with reduced config
- [x] 5.5 Verify frontend API contract unchanged — run `make web-typecheck && make web-build`

## Phase 6: Database and Kafka consolidation

- [x] 6.1 Update `deploy/postgres/01-create-databases.sql` from 10 databases to 4:
    - `identity_db`, `workspace_db`, `agent_db`, `runner_db`
- [x] 6.2 Reduce Kafka topic catalog from 21 to 9 surviving topics:
...
- [x] 6.3 Update Kafka consumer groups to match surviving topics only
- [x] 6.4 Run `go vet ./... && go build ./...` — verify compilation
- [x] 6.5 Run Kafka integration tests if `AAKS_KAFKA_TEST_BROKERS` is set
...
- [x] 7.6 Verify no critical bugs: confirm the credential-less-sandbox invariant holds (no provider keys or git tokens reach container env/filesystem/logs)