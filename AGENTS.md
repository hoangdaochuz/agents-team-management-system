...
- **`infra/`** Terraform: modules (`vpc`, `gke` incl. tainted `aaks/sandbox` node pool for DinD, `cloudsql` 4 logical DBs private-IP, `managed-kafka` the 9 event topics + `.dlq` twins, `filestore` clone root, `secrets`, `wif` GitHub-OIDC + Workload Identity) with `envs/dev` and `envs/prod`. Runbook: `infra/README.md`.
- **`k8s/`** Kustomize: `base/` (gateway/identity/workspace/agent/web + PreSync migration jobs), `sandbox/base/` (executor with DinD sidecar + Filestore `/clone` PVC + clone-bootstrap CronJob, in the dedicated `aaks-sandbox` namespace — PSS `restricted` cannot admit the privileged DinD sidecar next to services), `overlays/{dev,prod}` + `sandbox/overlays/{dev,prod}` (image digests, ingress+managed cert, env ConfigMaps), `components/` (Argo CD app-of-apps + pinned Helm operator apps, Kyverno policies incl. verify-images, External Secrets via ClusterSecretStore, tuned Falco/Trivy values, NetworkPolicies, PSS). Layout/promotion/bootstrap: `k8s/README.md`. All four overlays must keep `kustomize build` clean.
- **CI/CD** `.github/workflows/`: `ci.yml` (master; Semgrep + Gitleaks + Dependabot + existing go/frontend gates + terraform/kustomize/Trivy-config validation) and `deploy.yml` (build-once matrix of 9 images → Trivy CRITICAL gate → Syft SBOM → Cosign keyless sign → promote dev overlays (workloads + sandbox) **by digest**, sandbox base digest via env ConfigMap; prod via manual PR). Repo VARIABLES needed: `GCP_WIF_PROVIDER`, `GCP_SA`, `GCP_REGISTRY`.
