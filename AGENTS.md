...
- **Event bus**: sarama client; topic catalog + event types in `backend/internal/contracts/events/`. All lifecycle/step topics are **partitioned by task_id** (ordered delivery per task); producer is idempotent; consumers are at-least-once with an idempotency hook. Task service is the **saga coordinator**: emits commands (`task.run-requested`, `task.review-requested`, `task.stop-requested`), consumes facts (`run.completed`, `verdict`, `pr.opened`, `task.status-changed`).
- **Secrets**: Settings service is the sole decryptor of provider keys (master key + internal token channel to Runner; dev certs in `deploy/certs/`). The credential-less-sandbox invariant carries over from the old design — never put API keys or git credentials in the container path.
...
## Deployment (GCP, DevSecOps — OpenSpec change `deploy-gcp-kubernetes-devsecops`)

- **`infra/`** Terraform: modules (`vpc`, `gke` incl. tainted `aaks/sandbox` node pool for DinD, `cloudsql` 10 logical DBs private-IP, `managed-kafka` all event topics + `.dlq` twins, `filestore` clone root, `secrets`, `wif` GitHub-OIDC + Workload Identity) with `envs/dev` and `envs/prod`. Runbook: `infra/README.md`.
- **`k8s/`** Kustomize: `base/` (12 workloads + PreSync migration jobs; runner has the DinD sidecar + Filestore `/clone` PVC), `overlays/{dev,prod}` (image digests, ingress+managed cert, env ConfigMaps), `components/` (Argo CD apps, Kyverno policies incl. verify-images, External Secrets, Trivy Operator, Falco, NetworkPolicies, PSS). Layout/promotion/bootstrap: `k8s/README.md`. Both overlays must keep `kubectl kustomize` clean.
- **CI/CD** `.github/workflows/`: `ci.yml` (master; Semgrep + Gitleaks + Dependabot + existing go/frontend gates + terraform/kustomize/Trivy-config validation) and `deploy.yml` (build-once matrix → Trivy CRITICAL gate → Syft SBOM → Cosign keyless sign → promote dev overlay **by digest**; prod via manual PR). Repo VARIABLES needed: `GCP_WIF_PROVIDER`, `GCP_SA`, `GCP_REGISTRY`.
- **Managed Kafka auth**: services speak SASL/PLAIN over TLS when `KAFKA_SASL_USER` is set (`internal/platform/kafka`, env `KAFKA_TLS`/`KAFKA_SASL_MECHANISM`); unset = plaintext compose path. K8s pulls credentials from the `kafka-credentials` ExternalSecret (`aaks-<env>-kafka-sasl-*` in Secret Manager).
- **Packaging**: `deploy/service.Dockerfile` (distroless, 10 services), `deploy/runner.Dockerfile` (alpine+git, runner only — it execs `git worktree` host-side), `backend/runner/Dockerfile` (sandbox base), `frontend/Dockerfile` + `frontend/nginx.conf` (nginx-unprivileged 8080, envsubst `AAKS_RESOLVER`/`AAKS_GATEWAY_UPSTREAM`). All base images digest-pinned — refresh with `make pin-digests`.
- Tasks 8.1–8.5 (live `terraform apply`, GKE bring-up, in-cluster verification) are pending — they need a real GCP project.
