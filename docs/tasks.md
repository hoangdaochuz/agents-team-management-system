...
> **STATUS: SUPERSEDED.** The backend is implemented as a consolidated event-driven system
> (5 services — gateway, identity, workspace, agent, executor) driven by the OpenSpec changes
> `openspec/changes/event-driven-microservices-backend/` and
> `openspec/changes/consolidate-microservices/` (authoritative). This file is the original
> single-binary task plan; it is kept for history. `AGENTS.md` documents the verified current state.
