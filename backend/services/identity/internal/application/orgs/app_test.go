...
func TestCreateWorkspaceAutoCreatesOrg(t *testing.T) {
	...
	if len(p.events) != 0 {
		t.Fatalf("workspace creation emits no events (provisioning is a direct call), got %v", p.events)
	}
}
...
func TestApproveOrgRequest(t *testing.T) {
	...
	if len(p.events) != 1 || p.events[0] != events.TopicSignupApproved {
		t.Fatalf("expected signup.approved only (provisioning is a direct call), got %v", p.events)
	}
}
