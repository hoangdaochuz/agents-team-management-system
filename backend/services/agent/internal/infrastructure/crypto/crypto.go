...
// a master key derived from AGENT_MASTER_KEY (SHA-256). Ciphertext is
...
func New(masterKey string) (*Cipher, error) {
	if masterKey == "" {
		return nil, errors.New("AGENT_MASTER_KEY not set")
	}
	if len(masterKey) < 16 {
		return nil, fmt.Errorf("AGENT_MASTER_KEY must be at least 16 bytes")
	}
