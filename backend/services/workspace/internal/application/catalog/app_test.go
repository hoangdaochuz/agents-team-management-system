...
import (
	...
	"github.com/aaks/server/services/workspace/internal/domain/catalog"
)
...
func TestCreateSkillPersists(t *testing.T) {
	...
	if len(p.events) != 0 {
		t.Fatalf("skill creation emits no events, got %v", p.events)
	}
}
...
func TestDeleteSkillPersists(t *testing.T) {
	app, f, _, p := newTestApp()
	ws := identity.ID("ws1")
	...
	if len(p.events) != 0 {
		t.Fatalf("skill deletion emits no events, got %v", p.events)
	}
}
...
func TestDeleteSkillScoped(t *testing.T) {
	...
	if len(p.events) != 0 {
		t.Fatalf("no events may be published on a rejected delete, got %v", p.events)
	}
}
