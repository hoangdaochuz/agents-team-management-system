...
func TestPatchStatusDoingPublishesRunRequested(t *testing.T) {
	...
	if len(p.events) != 1 || p.events[0].topic != events.TopicTaskRunRequested {
...
func TestPatchStatusDoingWithoutAgentBlocks(t *testing.T) {
	...
	if len(p.events) != 0 {
		t.Fatalf("blocked transition emits no saga events, got %v", p.topics())
	}
}
...
func TestPatchStatusStoppedPublishesStopRequested(t *testing.T) {
	...
	if len(p.events) != 1 || p.events[0].topic != events.TopicTaskStopRequested {
		t.Fatalf("expected stop-requested, got %v", p.topics())
	}
}
...
func TestRunCompletedAdvancesToReview(t *testing.T) {
	...
	want := []string{events.TopicTaskReviewRequested}
	if len(p.events) != 1 || p.events[0].topic != want[0] {
		t.Fatalf("expected review-requested, got %v", p.topics())
	}
	rv := p.events[0].data.(events.ReviewRequestedData)
...
func TestVerdictApprove(t *testing.T) {
	...
	if len(p.events) != 0 {
		t.Fatalf("approved task emits no saga events, got %v", p.topics())
	}
}
...
func TestVerdictRequestChangesRoundsExhausted(t *testing.T) {
	...
	if len(p.events) != 0 {
		t.Fatalf("blocked task emits no saga events, got %v", p.topics())
	}
}
...
func TestStopPublishesStopRequested(t *testing.T) {
	app, tf, _, p := newTestApp()
	seedTask(tf, "t1", tasks.TaskDoing, 2, agent("a1"))
