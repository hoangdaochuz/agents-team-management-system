// Package messaging adapts the Kafka bus to the Workspace application
// handlers: the task-lifecycle saga facts (run.completed/verdict/pr.opened)
// and the workspace.created projections (default project binding + default
// rule seeding) arriving from the Identity service.
package messaging
...
// New builds the messaging adapter from the given handlers. Subscribing to a
// new topic is passing one more handler here; the plumbing never changes.
...
// construction retries with backoff: services routinely race Kafka's readiness
// at boot, and a one-shot miss would silence the saga for the process lifetime.
func (c *Consumer) Start(ctx context.Context, brokers string) {
	if brokers == "" {
		return
	}
	go func() {
		cg, err := kafka.NewConsumerGroupUntilReady(ctx, kafka.Brokers(strings.Split(brokers, ",")), "workspace-saga", c.log)
		if err != nil {
			c.log.Warn("workspace consumers stopped before Kafka was reachable", "error", err)
			return
		}
		if err := cg.Run(ctx, c.topics, c.consume); err != nil {
			c.log.Error("workspace consumer stopped", "error", err)
		}
		_ = cg.Close()
	}()
}
...
func (c *Consumer) consume(ctx context.Context, msg events.EventEnvelope) error {
	...
	if !ok {
		c.log.Warn("workspace consumer dropping unhandled event", "type", msg.EventType, "task_id", msg.TaskID)
		return nil
	}
