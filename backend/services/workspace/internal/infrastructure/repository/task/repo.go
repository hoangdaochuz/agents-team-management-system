...
// SagaAdvance records (task_id, run_id) as processed and transitions the task
// from→to (setting round_no when setRound) in ONE statement, so the idempotency
// mark and the state change commit or fail together. This is the saga's
// idempotency hook for at-least-once Kafka redelivery: false when the run was
// already seen or the task is not in `from`, in which case nothing changes.
// The CTE INSERT still executes when the UPDATE matches 0 rows, so an
// out-of-order fact (e.g. run.completed for a task outside `from`) is marked
// seen exactly as the previous mark-then-guard sequence did.
func (r *Repo) SagaAdvance(ctx context.Context, taskID, runID identity.ID, from, to tasks.TaskStatus, roundNo int, setRound bool) (bool, error) {
	roundCol := ""
	args := []any{taskID, runID, to, from}
	if setRound {
		roundCol = ", round_no = $5"
	...
	tag, err := r.q.Exec(ctx, `
		...
		UPDATE tasks SET status = $3`+roundCol+`, updated_at = now()
		WHERE id = $1 AND status = $4 AND EXISTS (SELECT 1 FROM ins)`, args...)
	if err != nil {
		return false, err
	}
