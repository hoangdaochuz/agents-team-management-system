...
import (
	...
	"github.com/aaks/server/services/workspace/internal/domain/project"
)
...
func (r *Repo) Update(ctx context.Context, id identity.ID, ws []identity.ID, in domain.UpdateInput) (tasks.Project, error) {
	...
	if in.RepoSource != nil {
		sets = append(sets, fmt.Sprintf("repo_source = $%d", idx))
		args = append(args, *in.RepoSource)
		idx++
	}
	if in.RepoType != nil {
		sets = append(sets, fmt.Sprintf("repo_type = $%d", idx))
		args = append(args, *in.RepoType)
		idx++
	}
	if in.DefaultBranch != nil {
		sets = append(sets, fmt.Sprintf("default_branch = $%d", idx))
		args = append(args, *in.DefaultBranch)
		idx++
	}
	if in.ClonedPath != nil {
		sets = append(sets, fmt.Sprintf("cloned_path = $%d", idx))
		args = append(args, *in.ClonedPath)
	}
	if len(sets) == 0 {
