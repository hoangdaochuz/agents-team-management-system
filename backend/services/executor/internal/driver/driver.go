...
// E2E); the LLM driver is selected with EXECUTOR_DRIVER=llm.
package driver
...
// New returns the configured driver (EXECUTOR_DRIVER: simulated | llm). An
...
// default — dev/E2E must set EXECUTOR_DRIVER=simulated explicitly.
func New(driverName string, log *slog.Logger) Driver {
	...
	default:
		log.Warn("unknown EXECUTOR_DRIVER; defaulting to llm", "driver", driverName)
		return &LLMDriver{log: log}
	}
}
