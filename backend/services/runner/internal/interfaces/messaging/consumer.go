// Package messaging adapts the Kafka bus to the Runner application handlers
...
func (c *Consumer) consume(ctx context.Context, msg events.EventEnvelope) error {
	...
	if !ok {
		c.log.Warn("runner consumer dropping unhandled event", "type", msg.EventType, "task_id", msg.TaskID)
		return nil
	}
